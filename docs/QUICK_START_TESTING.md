...
## Step 9: Create Admin Account (Bootstrap)

The backend provides a one-time bootstrap endpoint to create (or promote) an admin user:

- **URL:** `POST http://localhost:5000/api/admin/bootstrap`
- **Header:** `X-Bootstrap-Key: <BOOTSTRAP_ADMIN_KEY>`

By default (Docker), `BOOTSTRAP_ADMIN_KEY` is set in `../docker-compose.yml`.

### PowerShell example

```powershell
$key = "smartmatch-bootstrap-2026"   # must match BOOTSTRAP_ADMIN_KEY
$uri = "http://localhost:5000/api/admin/bootstrap"
...
Invoke-RestMethod -Method Post -Uri $uri -Headers @{ "X-Bootstrap-Key" = $key } -ContentType "application/json" -Body $body
```
...
$loginBody = @{ email = "admin@dut.udn.vn"; password = "password123" } | ConvertTo-Json
Invoke-RestMethod -Method Post -Uri $loginUri -ContentType "application/json" -Body $loginBody
```

### ⚠️ Production safety notes

- Change `BOOTSTRAP_ADMIN_KEY` and do **not** use weak passwords.
- Disable bootstrap by removing `BOOTSTRAP_ADMIN_KEY` in production.
